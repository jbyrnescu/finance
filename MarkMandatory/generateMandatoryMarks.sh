...
rm ${generatedDir}/GMarkAsMandTXs.sql

for curBankAcct in $tble1 $tble2 $tble3
do
    cat ${HOME}/Dropbox/finance/MarkMandatory/MandatoryMap.csv | sed -E -e "s/^([^,]*),(.*)$/update $curBankAcct set Mandatory=\"\\2\" where Description like \"%\\1%\";/" >> ${generatedDir}/GMarkAsMandTXs.sql
